...
testMixStats <- function(){
  ...
  verbose <- TRUE
  verbose_IRLS <- TRUE
  mixOptions <- ModelOptions(n_tries, max_iter, threshold, verbose, verbose_IRLS, variance_types$hetereskedastic)

  mixStats <- MixStats(model, mixOptions)
  #print(mixStats$nu)
}
testMixStats()
